...
class CurrencyConverterApp:
    def __init__(self, root):
        ...
        self.from_currency_combo = ttk.Combobox(
            root, values=list(self.currency_mapping.values()), state="readonly"
        )
    ...
    def convert_currency(self):
        ...
        from_currency_code = next(
            key
            for key, value in self.currency_mapping.items()
            if value == from_currency_name
        )
        to_currency_code = next(
            key
            for key, value in self.currency_mapping.items()
            if value == to_currency_name
        )
        ...
        self.result_label.config(
            text=f"{amount:.2f} {from_currency_name} is equal to {converted_amount:.2f} {to_currency_name}",
